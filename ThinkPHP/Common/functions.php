...
function U($url='',$vars='',$suffix=true,$domain=false) {
    ...
    if($crlMode == '0')
    {
        if(empty($url))
        {
            ...
            $defaultModule =  C('DEFAULT_MODULE');
            $defaultController =  C('DEFAULT_CONTROLLER');
            $defaultAction =  C('DEFAULT_ACTION');
            return "index.php?m=$defaultModule&c=$defaultController&a=$defaultAction";
        }
        $url = rtrim($url,'/');
        $params = explode('/',$url);
        //var_dump($params);
