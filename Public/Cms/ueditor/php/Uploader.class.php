 ...
 * UEditor编辑器通用上传类
 */
require($_SERVER['DOCUMENT_ROOT'].'/Addons/Tencentyun/include.php');
...
//        "POST" => "文件大小超出 post_max_size 限制" ,
    ...
    private function upFile( $base64 )
    {
		...
		// 上传图片到云端
		$bucket = 'otherfx'; // 自定义空间名称，
		$fileid = 'pic'.time();  // 自定义文件名
		$uploadRet = ImageV2::upload($this->fullName, $bucket, $fileid);

		if (0 === $uploadRet['code']) {
			//云端上传成功 删除本地图片
			unlink($this->fullName);
			//取得云端图片真实路经
			$this->fullName = $uploadRet['data']['downloadUrl'];
		}
		else {
			//云端上传失败 完整根目录路经(因为已经修改ueditor.config.js 中 imagePath为空)
			$this->fullName = '/public/cms/ueditor/php/'.$this->fullName;
		}
    }
